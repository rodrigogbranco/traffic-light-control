...
class BaseOrchestrator2:
    ...
    def open_adj_tls(self,tl,phase,numberOfPhases):
        self.active_adj_tls[tl] = { 'phase' : phase, 'numberOfPhases' : numberOfPhases, 'active' : False}

    def clear_adj_tls(self,tl):
        if tl in self.active_adj_tls:
            current_phase = self.mw.get_phase_of_tl(tl)
            self.mw.set_phase_of_tl(tl,(current_phase + 1) % self.active_adj_tls[tl]['numberOfPhases'])
            del self.active_adj_tls[tl]

    def open_tl_at_time_by_cycles(self,ev,cycle_quantity,tl_id,t,conf,infinity):
        ...
        self.commands[t_key][tl_id] = {}

        self.commands[t_key][tl_id]['ev'] = ev
        self.commands[t_key][tl_id]['conf'] = conf
        self.commands[t_key][tl_id]['infinity'] = infinity
        self.commands[t_key][tl_id]['cycles'] = cycle_quantity
        self.commands[t_key][tl_id]['start'] = int(self.mw.get_time())
        self.commands[t_key][tl_id]['color'] = 'y'
        self.commands[t_key][tl_id]['duration'] = -1

        if tl_id not in self.active_evs_by_tl:
            self.active_evs_by_tl[tl_id] = set()

        self.active_evs_by_tl[tl_id].add(ev)

    def open_tl_at_time_by_cycles_by_time(self,ev,cycle_quantity,tl_id,t,conf,duration):
        self.open_tl_at_time_by_cycles(ev,cycle_quantity,tl_id,t,conf,False)
        ...
        self.commands[t_key][tl_id]['duration'] = duration
    ...
    def execute_before_step_simulation(self,step):
        for adj_tl in self.active_adj_tls:
            if not self.active_adj_tls[adj_tl]['active']:
                current_phase = self.mw.get_phase_of_tl(adj_tl)
                state = self.mw.get_rgb_state(adj_tl)

                if (self.active_adj_tls[adj_tl]['phase'] == 'red' and np.all(list(state.upper()) == 'R')) or \
                    (self.active_adj_tls[adj_tl]['phase'] == current_phase):
                        current_phase = self.mw.set_phase_duration_of_tl(adj_tl,1000000)
                        self.active_adj_tls[adj_tl]['active'] = True
                elif 'G' in state.upper():
                    self.mw.set_phase_of_tl(adj_tl,(current_phase + 1) % self.active_adj_tls[adj_tl]['numberOfPhases'])
        ...
        if len(self.commands) > 0 and step in self.commands:
            for tl_id in self.commands[step]:
                self.actuate(step, tl_id)
                #tlname = self.commands[step][edge]['conf'].edges[edge]['name']
                tlname = tl_id
                ...
                cur_command = self.commands[step][tl_id]
                ...
                if not cur_command['infinity']:
                    if cur_command['duration'] != -1:
                        ...
                        total_time = int(cur_command['cycles']*cur_command['conf'].tls_conf[tlname]['g']['duration'])
    ...
    def actuate(self,step, tl_id):
        command = self.commands[step][tl_id]
        conf = command['conf']
        infinity = command['infinity']
        #tl = conf.edges[edge]['name']
        tl = tl_id
        if command['color'] == 'y':
            ...
            if lights.lower().count('r') == len(lights):
                self.mw.set_phase_of_tl(tl,conf.tls_conf[tl]['g']['index'])
                if infinity:
            ...
            if self.mw.get_phase_of_tl(tl) == conf.tls_conf[tl]['g']['index']:
                my_green_phase = True
                if infinity:
                    self.mw.set_phase_of_tl(tl,conf.tls_conf[tl]['g']['index'])
                    self.mw.set_phase_duration_of_tl(tl,1000000)
            ...
            if not my_green_phase:
                self.change_y_phase(lights, tl, conf.tls_conf[tl], step)

            next_switch = int(self.mw.get_next_switch_of_tl(tl))

            cycles = command['cycles']
            red_phase_duration = int(conf.tls_conf[tl]['r']['duration'])
            green_phase_duration = int(conf.tls_conf[tl]['g']['duration'])
            ...
            for c in range(1,int(cycles)+1):
                ...
                if tl not in self.commands[next_times]:
                    self.commands[next_times][tl] = {}

                self.commands[next_times][tl]['color'] = 'g'
                self.commands[next_times][tl]['ev'] = command['ev']
                self.commands[next_times][tl]['conf'] = command['conf']
                self.commands[next_times][tl]['infinity'] = command['infinity']
                self.commands[next_times][tl]['cycles'] = command['cycles']
                self.commands[next_times][tl]['start'] = command['start']
                self.commands[next_times][tl]['duration'] = command['duration']

                next_times = next_times + green_phase_duration
        else:
            self.logger.debug(tl+' '+ str(conf.tls_conf[tl][command['color']]['index']))
            self.logger.debug(self.mw.get_complete_definition_of_tl(tl))
            self.mw.set_phase_of_tl(tl,conf.tls_conf[tl][command['color']]['index'])
            if infinity:
                self.mw.set_phase_duration_of_tl(tl,1000000)
    ...
    def sync_tls(self, step):
        ...
        for tl_to_sync in self.tls_to_sync:
            data = self.tls_to_sync[tl_to_sync]
            tl_info_removed = data['conf'].tls_conf[tl_to_sync]
            ...
            data['state'] = self.change_state(data['state'], tl_name, \
		                                                    real_index, tl_name, remove_from_sm, total_phases, \
		                                                    tl_info_removed, time_in_program, step, data['infinity'])

        for tl in remove_from_sm:
            data = self.tls_to_sync[tl]
            del self.tls_to_sync[tl]
            #tl = data['conf'].edges[edge]['name']
            ...
            if data['infinity']:
                steps = [s for s in self.commands if tl in self.commands[s]]
                for s in steps:
                    del self.commands[s][tl]
    ...
    def schedule_sync(self,ev,tl_id,conf,infinity):
        self.tls_to_sync[tl_id] = {}
        self.tls_to_sync[tl_id]['state'] = 1
        self.tls_to_sync[tl_id]['ev'] = ev
        self.tls_to_sync[tl_id]['conf'] = conf
        self.tls_to_sync[tl_id]['infinity'] = infinity

        if tl_id in self.active_evs_by_tl and ev in self.active_evs_by_tl[tl_id]:
            self.active_evs_by_tl[tl_id].remove(ev)
