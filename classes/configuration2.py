  ...
  def compute_values(self):
    ...
    for i in range(index,len(self._edges_order)):
      ...
      if i+1 < len(self._edges_order):
        if edge_name in tlsjson:
          if self._edges_order[i+1] in tlsjson[edge_name]['edges']:
            conn_tls[i].append(tlsjson[edge_name]['edges'][self._edges_order[i+1]]['tl'])
    ...
    for i in range(index,len(self._edges_order)):
        ...
        if tls and i+1 < len(self._edges_order):
          ...
          for li in range(len(controlled_links)):
            ...
            if len(link_tuple) > 0:
              ...
              allowed = self._mw.get_allowed_vehicles(link_tuple[0][0])
              if edge_name in link_tuple[0][0] and self._edges_order[i+1] in link_tuple[0][1] and \
                (len(allowed) == 0 or my_type in allowed):
                tl = tls
                link_index = li
      ...
      if tl != None:
          ...
          if len(has_tl) > 0:
            ...
            for li in range(len(controlled_links)):
              ...
              if edge_name in link_tuple[0][0] and self._edges_order[i+1] in link_tuple[0][1]:
                allowed = self._mw.get_allowed_vehicles(link_tuple[0][0])
                if len(allowed) == 0 or my_type in allowed:
        ...
        current_edge['tl']['adj'] = {}
        self.tls_conf[current_edge['tl']['name']] = current_edge['tl']

        edges_of_tl = []

    for edge_name in self.edges_order:
      if edge_name in self.tlsjson and edge_name in self.edges and 'tl' in self.edges[edge_name]:
        for adj_edge in self.tlsjson[edge_name]['edges']:
          if adj_edge in self.tlsjson:
            for next_edge in self.tlsjson[adj_edge]['edges']:
              if 'tl' in self.tlsjson[adj_edge]['edges'][next_edge]:
                tl_adj = self.tlsjson[adj_edge]['edges'][next_edge]['tl']

                if tl_adj not in self.tls:
                  all_programs = self._mw.get_complete_definition_of_tl(tl_adj)
                  my_program = [ program for program in all_programs if program.programID == current_program ][0]  

                  right_index = int(self.tlsjson[adj_edge]['edges'][next_edge]['index'][0])
                  lights_sequence = list(map((lambda ls: ls.state[right_index].upper()), my_program.phases))

                  green_phase_index = lights_sequence.index('G')
                  self.edges[edge_name]['tl']['adj'][tl_adj] = {'phase' : green_phase_index, 'numberOfPhases' : len(my_program.phases)}

        for adj_edge in self.tlsjson[edge_name]['in']:
          if adj_edge not in self.edges_order:
            for next_edge in self.tlsjson[adj_edge]['edges']:
              if 'tl' in self.tlsjson[adj_edge]['edges'][next_edge]:
                tl_adj = self.tlsjson[adj_edge]['edges'][next_edge]['tl']
                ...
                if tl_adj not in self.tls:
                  self.edges[edge_name]['tl']['adj'][tl_adj] = {'phase' : 'red', 'numberOfPhases' : len(my_program.phases)}         
