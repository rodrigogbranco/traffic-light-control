...
class TimedPetriStrategy(PreemptionStrategy):
  def configure(self):
      ...
      for tl_p0 in p0_marks:
        ...
        if tl_p0 not in self.preemptive:
          ...
          if qcurrent >= tl_info['qmax'] and self.mw.get_phase_of_tl(tl_p0) != tl_info['g']['index']:
            ...
            if qcurrent <= dlast:
              open_duration = tlast + math.sqrt((2*qcurrent)/self.a) + tflush
            else:
              open_duration = tlast + (2*self.a*qcurrent)/lane_vmax + lane_vmax/2*self.a + tflush

            opendur = max(open_duration,tl_info['g']['duration'])

            if self.pn.transition('t0_{}'.format(tl_p0)).min_time > step + opendur:
              self.open_tl_at_time_by_cycles_by_time(1,tl_info['name'],step,opendur)
  ...
  def get_min_times(self, index, edges, step):
    ...
    for i in range(index,len(edges)):
      ...
      if edge in self.conf.edges_with_tl: 
        ...
        ev_max_speed = self.mw.get_vehicle_max_speed(self.ev)

        tflush = tl_info['y']['duration'] + tl_info['r']['duration']

        first_term = dtl/ev_max_speed - (qi*self.k)/self.s
        ...
        if first_term <= 0:
          ...
          open_offset = max((1-self.options.prt)*(first_term - (2*self.a*qi)/lane_vmax - lane_vmax/2*self.a - tflush),0)
