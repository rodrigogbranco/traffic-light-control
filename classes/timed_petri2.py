...
class TimedPetriStrategy2(PreemptionStrategy):
  def configure(self):
    ...
    if self.ev_entered and not self.ev_exited:
      ...
      if current_tl_of_step is not None:
        if current_tl_of_step != self.last_tl:
          ...
          if self.time_stopped >= (tl_info['g']['duration']/2):
  ...
  def get_min_times(self, index, edges, step):
    ...
    edge_of_ev = self.mw.get_edge_of_vehicle(self.ev)
    ev_length = self.mw.get_vehicle_length(self.ev)    
    for i in range(index,len(edges)):
      ...
      if edge in self.conf.edges_with_tl: 
        ...
        for e in tl_info['edges']:
          ...
          avg_veh_length = self.mw.get_avg_veh_length_edge(e)
          if edge_of_ev == e:
            avg_veh_length -= ev_length                          
          qi += max(hn*avg_veh_length,0)
        ...
        ev_max_speed = min(ev_nominal_max_speed,lane_vmax)
        #ev_curr_speed = self.mw.get_vehicle_speed(self.ev)
        #ev_acc = self.mw.get_vehicle_max_acc(self.ev)

        tflush = tl_info['y']['duration'] + tl_info['r']['duration']

        #arrtime = dtl/ev_max_speed if ev_curr_speed >= lane_vmax else (ev_max_speed - ev_curr_speed)/ev_acc + (dtl - (ev_max_speed**2 - ev_curr_speed**2)/(2*self.a))/ev_max_speed
        ...
        if first_term <= 0:
          ...
          open_offset = max((1-self.options.prt)*(first_term - lane_vmax/self.a - (qi-dlast)/lane_vmax - tflush),0)
