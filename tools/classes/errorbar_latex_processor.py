...
class ErrorBarLatexProcessor(BaseLatexProcessor):
  ...
  def print_summary(self):
    ...
    for alg_args in result_output:
      ...
      if alg not in chosen_algs:
        ...
        chosen_algs[alg]['mean'] = result_output[alg_args][max_cars][0]
      elif self.best_worst == 'True' and result_output[alg_args][max_cars][0] > chosen_algs[alg]['mean'] or \
        self.best_worst == 'False' and result_output[alg_args][max_cars][0] < chosen_algs[alg]['mean']:
        chosen_algs[alg]['instance'] = alg_args
        chosen_algs[alg]['mean'] = result_output[alg_args][max_cars][0]
    ...
    for alg in algs_name:
      ...
      points = [ result_output[alg_name][n_veh][0] for n_veh in result_output[alg_name] ]
      ci = [ result_output[alg_name][n_veh][1] for n_veh in result_output[alg_name] ]

      nvehs = [ i+discount for i in n_veh_unique ]

      plt.errorbar(nvehs, points, yerr=ci, label=alg_name)
    ...
    if self.alg in self.results[self.location][i] and arg in self.results[self.location][i][self.alg]:
      ...
      if self.ev in self.results[self.location][i][self.alg][arg] and self.ev:
        ...
        if len(metric_value) > 0:
          ...
          interval = st.t.interval(0.95, len(metric_value)-1, loc=mean, scale=st.sem(metric_value))
